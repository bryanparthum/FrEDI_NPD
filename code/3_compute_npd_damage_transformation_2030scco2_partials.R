...
##          calculates the NPD value ($ per ton CO2, CH4, N2O) using 
##          1.5%, 2%, 2.5%, and 3% Ramsey discounting and a 2% and 3% constant discount rate
##          by 2300 from 2020, 2030, 2040 emission pulse in 2020 USD (marginal = perturbed - baseline).
##          - FrEDI annual damages for all pulse years are converted from 2015$ to 2020$
...
## Written by: US EPA, Office of Atmospheric Protection & National Center for Environmental Economics; January 2022
## Last updated: 5/31/2023 by E. McDuffie
## NOTES: 
#####
...
read_fredi = 
  function(x, GAS, YEAR){
    read_parquet(x) %>%
      ...
      select(sector, trial, year, damages.baseline, damages.perturbed) %>% 
      group_by(sector, trial, year) %>%     
...
for (GAS in perturb_gas) {
  for (YEAR in perturb_year ) {
    ...
    #dpath <- paste0('output/damages/rffsp/',GAS,'/damages_',GAS,'_',YEAR,'_')
    #only load data for the particular gas and year
    damages = 
      list.files(paste0('output/damages/rffsp'), pattern = paste0(GAS, '_', YEAR, '_'), full.names = T) %>% 
      map_df(~read_fredi(., GAS, YEAR))
    ...
    damages %<>% 
      group_by(sector, trial, gas, emissions.year) %>% 
    ...
    #   theme_bw()
    # 
    # ggsave(paste0('output/npd_damage_transformation/difference_between_direct_and_proportional__',GAS,'_',YEAR,'_damages.svg'), width = 9, height = 6) 
    # 
    # dif %>%
    #   ggplot() +
    ...
    #   theme_bw()
    # 
    # ggsave(paste0('output/npd_damage_transformation/difference_between_direct_and_proportional_',GAS,'_',YEAR,'_damages_pct.svg'), width = 9, height = 6) 
    ...
    ## export full streams
    data %>% 
      write_parquet(paste0('output/npd_damage_transformation/', GAS, '_full_streams_', YEAR, '_national_', ADAPT, '_adaptation_sectoral.parquet'))
    ...
    means = 
      bind_rows(
        means, 
        data %>%
          group_by(sector, gas, emissions.year, discount.rate) %>%
...
means %>% 
  write_csv(paste0('output/npd_damage_transformation/npd_summary_stats_', GAS, '_sectoral.csv'))
